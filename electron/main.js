         ...
         return new Promise((resolve) => {
            ...
            exec('reg query HKCU\\Environment /v Path', (err, stdout) => {
                ...
                if (!err) {
                    ...
                    if (matches && matches[1]) currentPath = matches[1];
                }
                const paths = currentPath.split(';').filter(p => p && !p.startsWith(basePath) && p.trim() !== '');
                paths.unshift(newPhpPath);
                const newPathString = paths.join(';');
                exec(`setx PATH "${newPathString}"`, (error) => {
                    if (error) {
                        resolve({ error: `Failed to set PATH. Try running as Administrator. Details: ${error.message}` });
                        return;
                    }
                    resolve({ success: `PHP ${version} is now active!` });
                });
            });
        });
    }
